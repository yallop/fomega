...
and isconstbinding fi ctx x =
  match getbinding fi ctx x with VarBind _ -> true | _ -> false
...
and isvalbinding fi ctx x =
  match getbinding fi ctx x with VarDef (t, _) -> isval ctx t | _ -> false

let rec expand ctx t =
  match t with
  | TmVar (fi, n, _) -> begin
    match getbinding fi ctx n with VarDef (t, _) -> expand ctx t | _ -> t
  end
  | _ -> t
...
let rec eval1 ctx t =
  ...
  | TmInj (fi, ty, i, tm) ->
    let tm' = eval1 ctx tm in
    TmInj (fi, ty, i, tm')
  | TmVar (fi, n, _) when isvalbinding fi ctx n -> raise NoRuleApplies
  | TmVar (fi, n, _) -> begin
...
let rec tyeqv ctx tyS tyT =
  ...
  match (tyS, tyT) with
  | TyVar (i, _), _ when istydefined ctx i -> tyeqv ctx (gettydef ctx i) tyT
  | _, TyVar (i, _) when istydefined ctx i -> tyeqv ctx tyS (gettydef ctx i)
  ...
  | TyProd tms1, TyProd tms2 ->
    ...
    && List.for_all2 (fun ty1 ty2 -> tyeqv ctx ty1 ty2) tms1 tms2
...
let rec kindof ctx tyT =
  match tyT with
  | TyVar (i, _) ->
    let knK = getKindFromContext dummyinfo ctx i in
    knK
  | TyAll (tyX, knK1, tyT2) ->
  ...
  | TyProd tys ->
    List.iter
      (fun ty ->
        if kindof ctx ty <> KnStar then error dummyinfo "kind * expected" )
      tys;
    KnStar
  | TySum (tyT1, tyT2) ->
    if kindof ctx tyT1 <> KnStar then error dummyinfo "kind * expected";
...
let rec typeof ctx t =
  ...
  | TmPac (fi, ty1, tm, x, k, ty2) ->
    ...
    if knTy1 <> k then error fi "Pack kind mismatch";
    let subst = typeSubstTop ty1 ty2 in
    if not (tyeqv ctx subst tyTm) then error fi "Pack type mismatch"
    else TyExi (x, k, ty2)
  | TmOpe (fi, t1, x, y, t2) -> (
    let tyT1 = typeof ctx t1 in
    match simplifyty ctx tyT1 with
  ...
  | TmCase (fi, tm1, x, tm2, y, tm3) -> (
    ...
    | TySum (tyL, tyR) ->
      ...
      in
      if not (tyeqv ctx ltype rtype) then
        error fi "Branches do not have the same type"
      else ltype
    | _ -> error fi "Variant type expected" )
  | TmInj (_fi, ty, L, t) ->
    TySum (simplifyty ctx (typeof ctx t), simplifyty ctx ty)
  | TmInj (_fi, ty, R, t) ->
    TySum (simplifyty ctx ty, simplifyty ctx (typeof ctx t))

(* -------------------- Prettyfy -------------------------- *)

(* Simplify type applications within expressions for better printing *)

let ap1 g x f = match g x with None -> None | Some x -> Some (f x)
...
let apn g xs f =
  let one x (xs, changed) =
    match g x with None -> (x :: xs, changed) | Some x -> (x :: xs, true)
  in
...
let rec prettifyty' tyT =
  ...
  | TyVar _ -> None
  | TyArr (ty1, ty2) ->
    ap2 prettifyty' ty1 ty2 (fun ty1 ty2 -> TyArr (ty1, ty2))
  | TyAll (s, k, ty) -> ap1 prettifyty' ty (fun ty -> TyAll (s, k, ty))
  | TyExi (s, k, ty) -> ap1 prettifyty' ty (fun ty -> TyExi (s, k, ty))
  | TyAbs (s, k, ty) -> ap1 prettifyty' ty (fun ty -> TyAbs (s, k, ty))
  | TyApp (ty1, ty2) ->
    ap2 prettifyty' ty1 ty2 (fun ty1 ty2 -> TyApp (ty1, ty2))
  | TyProd tyl -> apn prettifyty' tyl (fun tyl -> TyProd tyl)
  | TySum (ty1, ty2) ->
    ap2 prettifyty' ty1 ty2 (fun ty1 ty2 -> TySum (ty1, ty2))

let rec prettifyty tyT =
  match prettifyty' tyT with None -> tyT | Some tyT -> prettifyty tyT
...
let rec prettify t =
  match t with
  | TmVar _ -> t
  | TmAbs (fi, s, ty, t) -> TmAbs (fi, s, prettifyty ty, prettify t)
  | TmApp (fi, t1, t2) -> TmApp (fi, prettify t1, prettify t2)
  | TmPac (fi, ty1, t, s, kn, ty2) ->
    TmPac (fi, prettifyty ty1, prettify t, s, kn, prettifyty ty2)
  | TmOpe (fi, t1, s1, s2, t2) -> TmOpe (fi, prettify t1, s1, s2, prettify t2)
  | TmTAbs (fi, s, kn, t) -> TmTAbs (fi, s, kn, prettify t)
  | TmTApp (fi, t, ty) -> TmTApp (fi, prettify t, prettifyty ty)
  | TmProd (fi, ts) -> TmProd (fi, ts)
  | TmProj (fi, t, i) -> TmProj (fi, prettify t, i)
  | TmCase (fi, t1, s1, t2, s2, t3) ->
    TmCase (fi, prettify t1, s1, prettify t2, s2, prettify t3)
  | TmInj (fi, ty, inj, term) -> TmInj (fi, prettifyty ty, inj, term)

let prettifybinding bind =
  match bind with
  | NameBind -> bind
  | TyVarBind _ -> bind
