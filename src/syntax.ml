...
let typeShiftAbove d c tyT =
  tymap
    (fun c x n ->
      if x >= c then
        if x + d < 0 then err "scoping error" else TyVar (x + d, n + d)
      else TyVar (x, n + d) )
    c tyT
...
let getKindFromContext fi ctx i =
  ...
  | _ ->
    error fi
      ( "getKindFromContext: Wrong kind of binding for variable "
      ^ index2name fi ctx i )
...
let index_glyph idx =
  let rec loop i = if i = 0 then [] else loop (i / 10) @ [ i mod 10 ] in
  ...
  List.iter (fun i -> Buffer.add_string buf (digit_glyph i)) digits;
  Buffer.contents buf
...
and printty_SumType outer ctx tyT =
  match tyT with
  | TySum (tyL, tyR) ->
    obox0 ();
...
and printty_ProdType outer ctx tyT =
  match tyT with
  | TyProd tys ->
    let rec loop l =
      match l with
